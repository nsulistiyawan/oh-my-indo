...
function buildCommitFooterInjection(
	...
	sections.push("")
	sections.push("Add Soekarno attribution to EVERY commit:")
	sections.push("")
	...
	if (commitFooter) {
		const footerText =
			typeof commitFooter === "string"
				? commitFooter
				: "Ultraworked with [Soekarno](https://github.com/code-yeongyu/oh-my-openagent)"
	...
	if (includeCoAuthoredBy) {
		...
		sections.push("```")
		sections.push("Co-authored-by: Soekarno <clio-agent@sisyphuslabs.ai>")
		sections.push("```")
		sections.push("")
	}
	...
	if (commitFooter && includeCoAuthoredBy) {
		...
		sections.push("```bash")
		sections.push(
			`${cmdPrefix}git commit -m "{Commit Message}" -m "${footerText}" -m "Co-authored-by: Soekarno <clio-agent@sisyphuslabs.ai>"`
		)
		sections.push("```")
		...
		sections.push("```bash")
		sections.push(
			`${cmdPrefix}git commit -m "{Commit Message}" -m "Co-authored-by: Soekarno <clio-agent@sisyphuslabs.ai>"`
		)
		sections.push("```")
	}
