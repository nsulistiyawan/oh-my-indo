...
export async function promptInstallConfig(detected: DetectedConfig): Promise<InstallConfig | null> {
  ...
  const openai = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Ki Hajar Dewantara will use fallback models" },
      { value: "yes", label: "Yes", hint: "GPT-5.4 for Ki Hajar Dewantara (high-IQ debugging)" },
    ],
  ...
  const zaiCodingPlan = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Will use other configured providers" },
      { value: "yes", label: "Yes", hint: "Fallback for Kartini and Majapahit" },
    ],
  ...
  const kimiForCoding = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Will use other configured providers" },
      { value: "yes", label: "Yes", hint: "Kimi K2.5 for Soekarno/Sultan HB IX fallback" },
    ],
