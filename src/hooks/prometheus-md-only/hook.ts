    ...
    "tool.execute.before": async (
      ...
      if (!filePath) {
       ...
       if (!isAllowedFile(filePath, ctx.directory)) {
         log(`[${HOOK_NAME}] Blocked: Sultan HB IX can only write to .sisyphus/*.md`, {
           sessionID: input.sessionID,
           tool: toolName,
           filePath,
           agent: agentName,
         ...
         throw new Error(
           `[${HOOK_NAME}] Sultan HB IX is a planning agent. File operations restricted to .sisyphus/*.md plan files only. Use task() to delegate implementation. ` +
