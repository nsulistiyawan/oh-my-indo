...
function buildDynamicOrchestratorPrompt(ctx?: OrchestratorContext): string {
  ...
  const agentIdentity = buildAgentIdentitySection(
    "Soeharto",
    "Master Orchestrator agent from OhMyIndo that coordinates specialized agents to complete todo lists",
  )
...
export function createAtlasAgent(ctx: OrchestratorContext): AgentConfig {
  const baseConfig = {
    description:
      "Orchestrates work via task() to complete ALL tasks in a todo list until fully done. (Soeharto - OhMyIndo)",
    mode: MODE,
