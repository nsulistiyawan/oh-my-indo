 ...
 *
 * NOTE: Phrasing is carefully chosen to avoid triggering content filters on China-based
...
export function buildOracleSection(agents: AvailableAgent[]): string {
  ...
  return `<Oracle_Usage>
## Ki Hajar Dewantara - Read-Only High-IQ Consultant

Ki Hajar Dewantara is a read-only, expensive, high-quality reasoning model for debugging and architecture. Consultation only.

### WHEN to Consult (Ki Hajar Dewantara FIRST, then implement):
...
### Usage Pattern:
Briefly announce "Consulting Ki Hajar Dewantara for [reason]" before invocation.

**Exception**: This is the ONLY case where you announce before acting. For all other work, start immediately without status updates.

### Ki Hajar Dewantara Background Task Policy:

**Collect Ki Hajar Dewantara results before your final answer. No exceptions.**

**Ki Hajar Dewantara-dependent implementation is BLOCKED until Ki Hajar Dewantara finishes.**

- If you asked Ki Hajar Dewantara for architecture/debugging direction that affects the fix, do not implement before Ki Hajar Dewantara result arrives.
- While waiting, only do non-overlapping prep work. Never ship implementation decisions Ki Hajar Dewantara was asked to decide.
- Never "time out and continue anyway" for Ki Hajar Dewantara-dependent tasks.

- Ki Hajar Dewantara takes minutes. When done with your own work: **end your response** - wait for the \`<system-reminder>\`.
- Do NOT poll \`background_output\` on a running Ki Hajar Dewantara. The notification will come.
- Never cancel Ki Hajar Dewantara.
</Oracle_Usage>`
}
