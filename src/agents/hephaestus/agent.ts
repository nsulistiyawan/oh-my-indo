...
function buildDynamicHephaestusPrompt(ctx?: HephaestusContext): string {
  ...
  const agentIdentity = buildAgentIdentitySection(
    "Gajah Mada",
    "Autonomous deep worker for software engineering from OhMyIndo",
  );
...
export function createHephaestusAgent(
  ...
  return {
    description:
      "Autonomous Deep Worker - goal-oriented execution with GPT Codex. Explores thoroughly before acting, uses explore/librarian agents for comprehensive context, completes tasks end-to-end. Inspired by AmpCode deep mode. (Gajah Mada - OhMyIndo)",
    mode: MODE,
    model,
    maxTokens: 32000,
    prompt,
